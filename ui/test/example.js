...
const calculateDues = (bills) => {
	...
	for (const bill of bills) {
		for (const tab of bill.tabs) {
            if (expenditures[tab.name] === undefined) {
                expenditures[tab.name] = []
            }
            expenditures[tab.name].push({
                sponsor: bill.owner,
                ...tab  
            })
		}
...
// Calculate total cost of a meal for an event such as cold stones
const calculateBillSum = (bills) => {
    events = {}
    for (const bill of bills) {
        if (events[bill.eventName] === undefined) {
            events[bill.eventName] =  0
        }
        for (const tab of bill.tabs) {
            events[bill.eventName] += tab.price
        }
    }
    return events;
}
...
console.log(calculateBillSum(bills))
